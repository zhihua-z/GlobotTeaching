...
### 2.2 `/chat`  AI 法考对话助手（Stage 5 — 需求 1）
...
### 2.3 `/practice`  练习入口 + `/practice/[sessionId]` 练习中（Stage 5）
...
### 2.4 `/review`  今日 FSRS 复习（Stage 4+5 — 需求 5）
...
### 2.5 `/mistakes`  错题本（Stage 5 — Phase 2 强化）
...
### 2.7 `/dashboard`  自学看板（Stage 6 — 需求 6）
...
### 2.8 `/legal-articles`  法条速查 + `/legal-articles/[id]`（Phase 2）
...
### 2.9 `/questions/[id]`  题目详情 / 分析页（Stage 2 admin 切片，student 只读）
...
### 2.10 `/profile`  个人知识图谱可视化（Stage 4 — 需求 4）
...
### 3.2 `/admin/questions`  题库管理列表（Stage 1/2）
...
### 3.3 `/admin/questions/new` & `/admin/questions/[id]/edit`（Stage 1）
...
### 3.4 `/admin/questions/import`（Stage 1）
...
### 3.5 `/admin/review`  审核队列（Stage 3 — 需求 3）
...
### 3.8 `/admin/legal-articles`（Stage 1，配合 M0）
...
### 3.9 `/admin/pipeline`  流水线监控（Stage 3）
...
### 3.10 `/admin/prompts`  System Prompt 模板管理（Stage 2）
...
### 3.11 `/admin/evals`  Eval Set 管理（Stage 3）
...
- `QuestionStem` `OptionsBlock` `AnswerBlock` `RubricBlock` `SolutionBlock` `SimilarList` `DifficultyChart` `CitedArticlesBlock`（新增）

### 6.2 `components/practice/*`

- `PracticeRunner` `AnswerInput`（按 type 切换） `ResultPanel` `SubjectChips`

### 6.3 `components/review/*`

- `FsrsCard` `RatingButtons` `QueueProgressBar`

### 6.4 `components/chat/*`

- `ChatPane` `MessageBubble` `CitationChip`（法条 / 题目） `SlashCommandMenu`

### 6.5 `components/dashboard/*`

- `Heatmap`（recharts 自绘） `WeaknessList` `TrendsLine` `CountdownCard` `StreakBadge`

### 6.6 `components/profile/*`

- `L1Summary` `L2Markdown` `L3TopicGrid` `L4EventList`

### 6.7 `components/admin/*`

- `QuestionTable` `QuestionFilters` `QuestionForm` `BulkImportDialog`
- `ReviewQueueList` `ReviewCard`（含快捷键 hook）
- `TaxonomyTree` `LegalArticleForm`
- `PipelineRunList` `PipelineRunDetail` `PromptEditor`（Monaco + YAML schema）
- `EvalRunTable` `FailedCaseDrawer`

### 6.8 `components/ui/*`（shadcn 生成）

- `Button` `Input` `Select` `Tabs` `Dialog` `Sheet` `Tooltip` `Toast` `Skeleton` `Pagination` `Command`（CMD+K）

---

## 7. 页面 ↔ 开发日志 Stage 映射（Cline 排期）

| Stage | 必须上的页面 | 必须上的 API domain |
| --- | --- | --- |
| **Stage 1 (W1-W2)** | `/admin` · `/admin/questions` · `/admin/questions/new|edit|import` · `/admin/taxonomy` · `/admin/legal-articles` · `/questions/[id]`（admin 视角先用） | Auth(stub) · Questions · Taxonomy · Legal Articles · Admin overview |
| **Stage 2 (W3-W4)** | `/login` · 全局 layout · `/admin/prompts` | Auth 正式 · Prompts · LLM 网关 |
| **Stage 3 (W5-W6)** | `/admin/review` · `/admin/review/[id]` · `/admin/pipeline` · `/admin/pipeline/[runId]` · `/admin/evals` | Pipeline · Admin Review · Evals |
| **Stage 4 (W7-W8)** | `/profile` · `/review` | Profile · Review(FSRS) |
| **Stage 5 (W9-W10)** | `/` 首页 · `/chat` · `/practice` · `/practice/[id]` · `/progress` · `/mistakes` · `/settings` | Chat · Practice · Progress · Recommend · Mistakes · Settings |
| **Stage 6 (W11-W12)** | `/dashboard` · `/dashboard/reports/[week]` | Dashboard · Reports · 推送 cron |
| **Phase 2** | `/legal-articles` · `/legal-articles/[id]` · 多模态拍题上传 | Legal Articles 公开端 · 拍题匹配 |
